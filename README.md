# FlowCapital 💰

<div align="center">

![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![Compose](https://img.shields.io/badge/Compose-02569B?style=for-the-badge&logo=jetpackcompose&logoColor=white)
![License](https://img.shields.io/badge/License-MIT-blue?style=for-the-badge)

[![CI develop](https://github.com/FlowHack/flow-capital-android/actions/workflows/ci.yml/badge.svg?branch=develop)](https://github.com/FlowHack/flow-capital-android/actions/workflows/ci.yml)
[![CI master](https://github.com/FlowHack/flow-capital-android/actions/workflows/ci.yml/badge.svg?branch=master)](https://github.com/FlowHack/flow-capital-android/actions/workflows/ci.yml)

**Android-приложение для учёта финансовых потоков**

[Описание](#описание) • [Функции](#функции) • [Технологии](#технологии) • [Сборка](#сборка) • [Структура](#структура-проекта)

</div>

---

## 🤖 AI-Assisted Development

Проект спроектирован и написан с помощью ИИ в окружении OpenCode моделью Big Pickle

## Описание

**FlowCapital** — мобильное приложение для управления личными финансами с использованием системы "финансовых потоков". Приложение помогает отслеживать начисления, взносы и прогнозировать доходность по различным типам инвестиционных стратегий.

### Поддерживаемые потоки

| Поток | Описание | Статус |
|-------|---------|--------|
| **Растущий Поток (РП)** | Прогрессивный рост начислений (+0.003/клик) | ✅ |
| **Растущий Поток VIP (РП VIP)** | РП с повышенными коэффициентами (старт 0.3%, +25 п.п. к бонусам) | ✅ |
| **Поток Новичка (ПН)** | Стабильные 2% в день + 50% бонус | ✅ |
| **Премиум Стартовый Поток (ПСП)** | 20 периодов × 14 дней с растущими коэффициентами | ✅ |

---

## Функции

### 📊 Основные возможности

- **Управление потоками** — создание, отслеживание и завершение финансовых потоков
- **Ежедневные начисления** — автоматический расчёт и отображение доходности
- **Реинвест** — реинвестирование накоплений в новые потоки
- **Корректировка** — исправление ошибок в истории операций

### 📅 Планирование

- **Напоминания** — push-уведомления и будильники о необходимости действий
- **Календарь** — визуализация периодов и сроков
- **История** — полная история всех операций
- **Расчёт потоков** — прогнозирование будущих начислений

### 💾 Экспорт данных

- **Excel экспорт** — выгрузка данных для анализа в таблицах
- **Прогнозы** — предварительный расчёт доходности

### 🌐 Встроенный браузер

- Быстрый доступ к связанным сервисам (potok.cash, sberkossa, e-id)
- Поддержка прокси (SOCKS5, MTProto)
- Сохранение сессий и cookies между переключениями
- Перемещаемая кнопка обновления страницы (запоминает позицию)

### 📸 Сканер QR-кодов

- Встроенный сканер для быстрого доступа к ресурсам

### 🔄 Автообновления

- Проверка новых версий через GitHub API
- Уведомление и загрузка обновлений

---

## Технологии

### Ядро

| Технология | Версия | Назначение |
|------------|--------|------------|
| **Kotlin** | 2.3.20 | Основной язык разработки |
| **Jetpack Compose** | BOM 2026.03.01 | UI-фреймворк |
| **Material 3** | - | Дизайн-система |
| **Room** | 2.8.4 | Локальная база данных |
| **DataStore** | 1.2.1 | Хранение настроек |

### Архитектура

| Компонент | Описание |
|-----------|----------|
| **MVVM** | Архитектурный паттерн |
| **Clean Architecture** | Разделение на слои (UI → Domain → Data) |
| **StateFlow** | Реактивное управление состоянием |
| **Repository** | Паттерн для работы с данными |

### Дополнительные библиотеки

- **WorkManager** (2.11.2) — фоновые задачи и push-напоминания
- **AlarmManager** — точные будильники для напоминаний
- **FastExcel** (0.15.4) — экспорт в Excel формат
- **Navigation Compose** (2.9.7) — навигация между экранами
- **Retrofit** (2.12.0) + **Moshi** — сетевые запросы и сериализация
- **Coil** (2.7.0) — загрузка изображений
- **CameraX** (1.6.0) — работа с камерой и QR-кодами
- **Timber** (5.0.1) — логирование
- **Kotlinx Serialization** (1.7.3) — сериализация данных

---

## Сборка

### Требования

- Android Studio Hedgehog (2023.1.1) или новее
- JDK 17+
- Android SDK 28+ (Android 9)
- AGP 9.2.0

### Debug-сборка

```bash
# Клонирование репозитория
git clone https://github.com/FlowHack/flow-capital-android.git
cd flow-capital-android

# Debug-сборка
./gradlew assembleDebug

# APK будет в: app/build/outputs/apk/debug/
```

### Установка на устройство

```bash
adb install app/build/outputs/apk/debug/app-debug.apk
```

---

## Структура проекта

```
app/src/main/java/com/flowhack/flowcapital/
├── data/
│   ├── db/                          # Room база данных
│   │   ├── AppDatabase.kt            # Главная БД
│   │   ├── FlowEntity.kt             # Сущности потоков
│   │   ├── FlowDao.kt                # DAO интерфейсы
│   │   └── FlowRepository.kt         # Репозитории
│   ├── forecast/                     # Расчёты и прогнозы
│   │   ├── GrowingFlowForecast.kt    # Прогноз РП
│   │   ├── NoviceFlowForecast.kt     # Прогноз ПН
│   │   ├── PspFlowForecast.kt        # Прогноз ПСП
│   │   └── MissedDaysCalculator.kt   # Расчёт пропущенных дней
│   ├── settings/
│   │   └── SettingsManager.kt        # Управление настройками
│   ├── update/
│   │   ├── UpdateChecker.kt          # Проверка обновлений
│   │   └── ApkDownloader.kt          # Загрузка APK
│   ├── proxy/
│   │   ├── ProxyModels.kt            # Модели прокси
│   │   └── ProxyStorage.kt           # Хранение прокси
│   └── logging/
│       └── AppLogger.kt              # Логирование
├── notifications/
│   ├── ReminderWorker.kt             # WorkManager для push-напоминаний
│   ├── ReminderScheduler.kt          # AlarmManager — планирование будильников
│   └── AlarmReceiver.kt             # BroadcastReceiver для срабатывания будильника
├── AlarmActivity.kt                  # Экран будильника (MediaPlayer, поверх блокировки)
├── ui/
│   ├── screens/
│   │   ├── browser/                  # Встроенный браузер
│   │   │   └── BrowserScreen.kt
│   │   ├── calculator/               # Экраны расчётов
│   │   │   ├── CalculatorScreen.kt    # Главный экран (РП, ПН)
│   │   │   ├── PremiumStartScreen.kt # Экран ПСП
│   │   │   ├── FlowViewModel.kt       # ViewModel для РП и ПН
│   │   │   ├── PremiumStartViewModel.kt # ViewModel для ПСП
│   │   │   ├── FlowDialogs.kt        # Диалоги (старт, реинвест, прогноз)
│   │   │   ├── FlowComponents.kt     # UI компоненты (карточки, таблицы)
│   │   │   └── ScreenUtils.kt        # Утилиты экрана
│   │   └── settings/
│   │       └── SettingsScreen.kt     # Экран настроек
│   └── theme/
│       ├── Color.kt                  # Основные цвета
│       ├── FlowColors.kt             # Цветовая палитра потоков
│       ├── Theme.kt                  # Тема приложения
│       └── Type.kt                   # Типографика
└── MainActivity.kt                    # Точка входа
```

---

## Политика конфиденциальности

Приложение **FlowCapital** работает полностью оффлайн:

- ✅ Все данные хранятся локально на устройстве
- ✅ Не передаются на внешние серверы
- ✅ Не требуют регистрации или авторизации
- ✅ Единственный сетевой запрос — проверка версии через GitHub API

---

Copyright (c) 2026 FlowHack. Все права защищены.

---

<div align="center">

**Сделано с ❤️ для управления личными финансами**

</div>
